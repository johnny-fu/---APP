...
public class order extends AppCompatActivity implements DatePickerDialog.OnDateSetListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        TextView userpocket = (TextView) findViewById(R.id.textView12);
            ...
            public void run() {
                ...
                StringRequest stringRequest = new StringRequest(Request.Method.POST, url, new Response.Listener<String>() {
                    ...
                    public void onErrorResponse(VolleyError error) {
                        Toast.makeText(order.this, "error", Toast.LENGTH_LONG).show();
                    }
                }) {
                    @Override
                            ...
                            runOnUiThread(new Runnable() {
                                ...
                                public void run() {
                                    ...
                                    button4.setOnClickListener(new View.OnClickListener() {
                                        @Override
                                        public void onClick(View v) {
                                            datePickerDialog = DatePickerDialog.newInstance(order.this, Year, Month, Day);
    ...
    public void update(){
        ...
        new Thread(new Runnable(){
            @Override
            public void run() {
                String[] title = new String[db_search_meal()];
                String[] money = new String[db_search_meal()];
                String[] detail = new String[db_search_meal()];
                String[] order = new String[db_search_meal()];
                for (int i = 0; i < order.length; i++)
                    order[i] = "";
                try {
                    int j = 0;
                    Connection conn = null;
                    conn = DriverManager.getConnection(datasource, db_user, db_password);
                    Statement st = conn.createStatement();
                    String sql = "";
                    sql = "SELECT * FROM app.meal";
                    ResultSet rs = st.executeQuery(sql);
                    ResultSetMetaData rm = rs.getMetaData();
                    while (rs.next()) {
                        title[j] = String.valueOf(rs.getObject(2));
                        money[j] = String.valueOf(rs.getObject(4));
                        detail[j] = String.valueOf(rs.getObject(3));
                        j++;
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                ...
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        WordAdapter adapter = new WordAdapter(title,money,detail,order);
                        ListView list = (ListView) findViewById(R.id.listview);
                        list.setAdapter(adapter);
                    }
                });
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.N) {
            ...
            dialog.setOnDateSetListener(new DatePickerDialog.OnDateSetListener() {
                ...
                public void onDateSet(DatePicker view, int year, int month, int dayOfMonth) {
                    String jj = String.valueOf(month+1);
                    String kk = String.valueOf(dayOfMonth);
                    calendar.set(year, month, dayOfMonth);
                    SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd");
                    if (month + 1 < 10) jj = "0" + String.valueOf(month+1);
            ...
            calendar.add(Calendar.DATE, Integer.parseInt(orderd)-1);
            //抓資料庫   calendar.add(Calendar.DATE, day-1);
